# Technical Mapping: OME to BioStride

## Overview

This document provides detailed technical mappings between OME (Open Microscopy Environment) data model elements and corresponding BioStride schema components, with code examples and implementation guidance.

## Core Entity Mappings

### Image Entities

#### OME Image → BioStride Image Classes
...
#### OME Instrument → BioStride Instrument
...
**BioStride Equivalent (extending for compatibility):**
```yaml
instruments:
...
#### OME Experiment → BioStride Study/ExperimentRun
...
#### OME 5D Data (XYZCT) → BioStride Multi-Frame Images
...
#### OME ROI → BioStride Annotation Extension
...
### OME-TIFF ↔ BioStride Integration

```python
# Pseudo-code for conversion
from bioformats import load_image_metadata
from biostride import FluorescenceImage

def ome_to_biostride(ome_tiff_path):
    """Convert OME-TIFF to BioStride data model"""
    
    # Read OME-XML from TIFF header
    ome_xml = load_image_metadata(ome_tiff_path)
    
    # Create BioStride image
...
def create_bioformats_reader():
    """Create Bio-Formats reader for BioStride images"""
    
    class BioStrideReader:
        def read_fluorescence_image(self, biostride_image):
            # Convert to OME model
            ome_image = {
                'ID': biostride_image.id,
                'Name': biostride_image.file_name,
                'AcquisitionDate': biostride_image.acquisition_date,
                'Pixels': {
                    'SizeX': biostride_image.dimensions_x,
                    'SizeY': biostride_image.dimensions_y,
                    'Channels': [{
                        'ExcitationWavelength': biostride_image.excitation_wavelength,
                        'EmissionWavelength': biostride_image.emission_wavelength,
                        'Name': biostride_image.fluorophore
                    }]
                }
            }
            return ome_image
...
| OME Path | BioStride Path | Type | Notes |
|----------|----------------|------|-------|
...
BioStride extends beyond OME for structural biology:

| BioStride Path | Purpose | No OME Equivalent |
|----------------|---------|-------------------|
...
### 1. Creating OME-Compatible BioStride Data

```yaml
# Template for OME-compatible fluorescence microscopy in BioStride
studies:
...
```python
# Upload BioStride images to OMERO
def upload_to_omero(biostride_study):
    import omero.gateway
    
    conn = omero.gateway.BlitzGateway(...)
    
    for image in biostride_study.images:
        if isinstance(image, FluorescenceImage):
            ...
            # Add BioStride-specific metadata as key-value pairs
            map_ann = omero.gateway.MapAnnotationWrapper()
            map_ann.setValue([
                ["biostride:study_id", biostride_study.id],
                ["biostride:sample_code", image.sample.sample_code],
                ["biostride:technique", "fluorescence"]
            ])
...
def validate_ome_compatibility(image):
    """Check if BioStride image can be exported to OME"""
...
```yaml
# OME-style plate/well structure in BioStride
studies:
  - id: study:hcs-001
...
This technical mapping demonstrates that BioStride can achieve strong interoperability with OME while extending the model for structural biology needs. Key implementation priorities include:
...
The alignment enables researchers to use familiar OME tools while benefiting from BioStride's structural biology capabilities.