# OME (Open Microscopy Environment) and BioStride Alignment

## Executive Summary

The Open Microscopy Environment (OME) provides established standards for biological imaging data, while BioStride extends these concepts to structural biology techniques. This document outlines how BioStride aligns with and complements OME standards, enabling interoperability between light/fluorescence microscopy and structural biology data.
...
## BioStride-OME Alignment

### Shared Concepts

BioStride intentionally aligns with OME in several fundamental areas:
...
BioStride extends OME concepts for structural biology:

#### 1. Multi-Technique Integration

While OME focuses on light microscopy, BioStride adds:
...
BioStride adds detailed biomolecular information:
```yaml
...
BioStride explicitly models computational workflows:
```yaml
WorkflowRun:
...
| OME Entity | BioStride Entity | Notes |
|------------|------------------|-------|
...
BioStride's `FluorescenceImage` and `OpticalImage` classes are designed to be OME-compatible:
...
BioStride data can interface with Bio-Formats through:
...
Study combining fluorescence microscopy (OME) with cryo-EM (BioStride):
...
Study:
  images:
    ...
    - id: cryoem-001
      _type: Image2D  # BioStride cryo-EM
...
Linking OME light microscopy with BioStride X-ray imaging:
...
images:
  ...
  - id: xrf-map
    ...
    beam_size: 2.0     # μm
    # BioStride-specific X-ray fluorescence
```

## Benefits of Alignment

### For Users
1. **Familiar concepts**: Users experienced with OME find similar patterns in BioStride
2. **Tool reuse**: Existing OME tools can process compatible BioStride data
...
2. **Interoperability**: Data exchange between OME and BioStride systems
...
- OMERO.scripts ↔ BioStride WorkflowRuns
...
- BioStride MRC/MAP files for 3D densities
...
- Develop OMERO.table definitions for BioStride metadata
...
BioStride's alignment with OME standards creates a bridge between traditional microscopy and structural biology data management. This alignment enables:
...
By building on OME's successful foundation while extending it for structural biology's unique needs, BioStride facilitates the integration of multi-scale, multi-modal biological imaging data essential for modern integrative structural biology.
